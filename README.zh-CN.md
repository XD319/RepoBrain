...
## 轻量偏好层（preference layer）

长期的 durable memory（`decision` / `gotcha` / `convention` / `pattern`）仍然是主线。除此之外，RepoBrain 在 `.brain/preferences/` 用「Markdown + frontmatter」单独存 **routing preference**：skill / workflow / task_class 指向、prefer / avoid / require_review、可选有效期与取代关系。它不是通用聊天记忆，而是给「对 skill / workflow 的自然语言反馈」一个比 durable memory 更低门槛、仍可审计的落点。

**与 memory 里 skill routing 字段的分工：** durable memory 的 frontmatter（`required_skills`、`recommended_skills`、`suppressed_skills`、触发器、`invocation_mode`、`risk_level` 等）是**静态声明层**：可 review、长期有效、随 Git 管理的 repo 策略。preference 是**动态倾向层**：录入更快，可用 `task_hints` / `path_hints` 收窄范围，并受 `status`、`valid_*`、`superseded_by` 约束。**routing engine** 把静态 memory 信号、符合条件的 preference，以及 task / path / 当前分支等上下文合成**唯一确定性**结果；**`invocation_plan`** 是对 adapter 稳定的**输出契约**（`required`、`prefer_first`、`optional_fallback`、`suppress`，以及在无法自动拍板时的 `blocked` / `human_review`）。

**决策优先级（从高到低）：** 阻断类 / 显式 suppress 结果 → 静态 `required_skills` → 负向 preference（`avoid`）→ 正向 preference（`prefer`）→ 静态 recommended → optional 与更软信号。冲突按显式本地规则处理：同一 skill 上静态 **suppress** 优先于 **preference.prefer**；静态 **required** 通常优先于 **preference.avoid**，除非分数过于接近（则升级人工研判）。同一 skill 多条 preference 按置信度加权合并（`updated_at` 较新的后合并入聚合）。`stale` 或已 `superseded` 的 preference 不参与决策。

内部实现（供维护者参考）：`static_memory_policy_input` + `preference_policy_input` + `task_context_input` → `routing_engine` → `invocation_plan` 渲染；JSON 可附带 `routing_explanation` 供机器消费。
