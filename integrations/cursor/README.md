...
2. The `alwaysApply: true` rule makes Cursor run `brain start` at session start automatically.
3. At phase boundaries, run `brain capture --task "<task>" --path <path>` to let local rules decide.
4. Pipe a markdown summary to `brain extract` or `brain reinforce` for manual flows.
...
5. Do not repeat the same capture suggestion in the same conversation turn.
