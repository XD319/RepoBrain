...
fresh conversation later in same session
  ...
  -> Copilot refreshes durable repo context
task becomes clear
...
3. If a fresh conversation starts later in that same session, refresh repo context with `brain inject --task "<task>" --path <path>`.
