...
import { clearSessionProfile, combinedPromoteText, defaultSessionProfile, getSessionProfilePath, loadSessionProfile, mergeHints, saveSessionProfile, upsertSkillRouting } from "../session-profile.js";
...
program
  .command("session-set")
  .description(
    "Store ephemeral session hints under `.brain/runtime/` (local-only; not durable repo knowledge).",
  )
  ...
  .action(async (
    ...
    const textLine = texts.map((t) => t.trim()).filter(Boolean).join("\n").trim();
    const hasFlags =
